...
const TataPamongSection = () => {
  ...
  const columns = [
      ...
      children: [
        {
          ...
          render: (text) =>
            text ? <Checkbox checked disabled /> : <Checkbox disabled />,
          align: "center",
        },
        {
          title: "Nasional",
    ...
    {
      title: "Tanggal Awal Kerja Sama",
      dataIndex: "tanggalawal",
      key: "tanggalawal",
      editable: true,
    },
    {
      title: "Tanggal Akhir Kerja Sama",
    ...
    {
      title: "Manfaat Bagi Program Studi",
      dataIndex: "manfaatbagiprogramstudi",
      key: "manfaatbagiprogramstudi",
      editable: true,
    },
  ]
        ...
        const processedData = jsonData
          .filter(
            (row, index) =>
              ...
              row &&
              row.length > 0 &&
              row[requiredColumns.lembagamitra]
          )
          .map((row, index) => {
            const rowData = {}
            for (const key in requiredColumns) {
              if (
                requiredColumns[key] !== null &&
                row[requiredColumns[key]] !== undefined
              ) {
...
>>>>>>> da7b33b (update: mainlayout)
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/api/section/1/${prodiId}`,
        data
      )
  ...
  return (
    <div className="p-5 bg-white rounded-lg shadow-md mt-24">
      <h2>1. Tata Pamong, Tata Kelola, dan Kerjasama</h2>
      <h3>a. Kerjasama</h3>
      <Upload
        beforeUpload={() => false}
