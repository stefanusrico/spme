    ...
    const fetchFormula = async () => {
      try {
        const response = await axiosInstance.get(
          `http://localhost:8000/api/rumus/${id}`
        )
    ...
    reader.onload = (e) => {
      try {
        ...
        let processedData = jsonData
          .filter(
            (row) => row && row.length > 0 && row[requiredColumns.lembagamitra]
          )
          .map((row, index) => {
            return {
        ...
        if (formula) {
          try {
            const hasilPerhitungan = evaluate(formula, {
              N1,
              N2,
              N3,
              a,
            ...
            console.log(
              "Hasil Perhitungan dari Rumus Backend:",
              hasilPerhitungan
            )
