...
import { updateUserProfile, fetchUserData, uploadFile } from "../../Elements/Profile/profile.action"
import {
  faUser,
  faCircleUser,
  faLock,
...
const EditUser = ({ title = "Edit User" }) => {
  ...
  const handleChange = async () => {
      try {
        ...
        try {
            console.log("gagal1");
            const response = await axios.put(`http://localhost:8000/api/users/${id}`, dataToUpdate, 
                {
                    headers: {
                      Authorization: `Bearer ${token}`, 
                    },
