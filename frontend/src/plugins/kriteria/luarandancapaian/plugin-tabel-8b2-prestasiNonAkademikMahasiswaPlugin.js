...
export class PrestasiNonAkademikMahasiswaPlugin extends BasePlugin {
  ...
  // ✅ Use dynamic base processing dengan konversi tanggal dan boolean
  ...
  // ✅ Process field value berdasarkan tipe
  ...
  mapPrestasiNonAkademikFields(sampleItem) {
    return {
  ...
  normalizeData(data) {
    return data.map((item) => {
      const result = { ...item }
      const fieldMap = this.mapPrestasiNonAkademikFields(result)
  ...
  // ✅ Dynamic validation dengan pemeriksaan boolean yang benar
  validateData(data) {
    const errors = []
    ...
    data.forEach((item, index) => {
      const fieldMap = this.mapPrestasiNonAkademikFields(item)
      ...
      // Hitung jumlah tingkat yang dipilih - orang tidak boleh memilih lebih dari satu tingkat
