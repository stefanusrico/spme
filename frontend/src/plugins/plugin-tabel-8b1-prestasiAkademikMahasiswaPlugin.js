...
const PrestasiAkademikMahasiswaPlugin = {
  ...
  processExcelData: async function (
    ...
    const processedData = filteredData.map((row, index) => {
      ...
      const defaultMapping = {
        ...
        prestasi_yang_dicapai: 6,
      };

      Object.keys(item).forEach(fieldName => {
        if (['key', 'selected', 'no'].includes(fieldName)) return;
        const colIndex = detectedIndices && detectedIndices[fieldName] !== undefined ? detectedIndices[fieldName] : defaultMapping[fieldName];
        ...
        else if (fieldName.startsWith("tingkat_")) item[fieldName] = typeof value === "boolean" ? value : typeof value === "string" ? ["ya", "yes", "v", "1", "true", "√", "✓"].includes(value.toLowerCase().trim()) : typeof value === "number" ? value > 0 : false;
  ...
  calculateScore: function (data, jumlahMahasiswaTS) {
    ...
    const NM = jumlahMahasiswaTS = 100;
      ...
      booleanFields.forEach((field) => {
        if (result[field] !== undefined && result[field] !== null) {
          if (typeof result[field] === 'string') {
            ...
            result[field] = ["V", "yes", "v", "1", "true", "√", "✓"].includes(norm);
          } else {
