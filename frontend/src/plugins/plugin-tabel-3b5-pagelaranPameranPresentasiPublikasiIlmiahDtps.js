...
const PagelaranPameranPresentasiPublikasiIlmiahDTPSPlugin = createPluginHandler(
  {
    ...
    scoreCalculator: async function (data, config, additionalData) {
      ...
      data.forEach((item) => {
        const ts2 =
          typeof item.ts_2_jumlah_judul === "number"
            ? item.ts_2_jumlah_judul
            : Number(item.ts_2_jumlah_judul) || 0
        const ts1 =
          typeof item.ts_1_jumlah_judul === "number"
            ? item.ts_1_jumlah_judul
            : Number(item.ts_1_jumlah_judul) || 0
        const ts =
          typeof item.ts_jumlah_judul === "number"
            ? item.ts_jumlah_judul
            : Number(item.ts_jumlah_judul) || 0

        const jumlah = ts2 + ts1 + ts

        const judul =
          typeof item.jenis_publikasi === "string"
            ? item.jenis_publikasi.toLowerCase()
            : ""
