...
const EkuivalenWaktuMengajarPenuhDosen = {
  ...
  normalizeData(data) {
    return data.map((item) => {
      ...
      const textFields = ["nama_dosen_dt", "dpts"]
  ...
  validateData(data) {
    ...
    data.forEach((item, index) => {
      ...
      const totalSKS =
        (parseFloat(
          item.ps_yang_diakreditasi_pendidikan_pembelajaran_dan_pembimbingan
        ) || 0) +
        (parseFloat(
          item.ps_lain_di_dalam_pt_pendidikan_pembelajaran_dan_pembimbingan
        ) || 0) +
        (parseFloat(
          item.ps_lain_di_luar_pt_pendidikan_pembelajaran_dan_pembimbingan
        ) || 0) +
        (parseFloat(
          item.penelitian_ekuivalen_waktu_mengajar_penuh_ewmp_pada_saat_ts_dalam_satuan_kredit_semester_sks
        ) || 0) +
        (parseFloat(
          item.pkm_ekuivalen_waktu_mengajar_penuh_ewmp_pada_saat_ts_dalam_satuan_kredit_semester_sks
        ) || 0) +
        (parseFloat(
          item.tugas_tambahan_dan_atau_penunjang_ekuivalen_waktu_mengajar_penuh_ewmp_pada_saat_ts_dalam_satuan_kredit_semester_sks
        ) || 0)
