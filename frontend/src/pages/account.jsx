  ...
  const handleRemoveProfilePicture = async () => {
    try {
      ...
      if (userData.profile_picture) {
        const removePromise = removeProfilePicture(userData.id)

        toast.promise(removePromise, {
          pending: "Menghapus foto profil...",
          success: "Foto profil berhasil dihapus",
          error: {
            render({ data }) {
              return `Gagal menghapus foto profil: ${data.message}`
            },
          },
        })
      ...
      toast.error(
        error.message || "Terjadi kesalahan saat menghapus foto profil"
      )
    }
  }
          ...
          error: {
            render({ data }) {
              if (
                ...
                data.response.data.errors.file
              ) {
                return `Gagal mengunggah foto: ${data.response.data.errors.file[0]}`
              }
              ...
              <img
                src={
                  ...
                  (userData.profile_picture
                    ? `http://localhost:8000/storage/${userData.profile_picture.replace(
                        /\\/g,
                        ""
                      )}`
                    : "/default-avatar.png")
