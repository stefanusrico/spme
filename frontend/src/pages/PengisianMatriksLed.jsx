...
const apiKey =
  "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"
...
const PengisianMatrikLed = () => {
  ...
  const checkAndRedirect = (userTasks) => {
    ...
    } else if (Array.isArray(userTasks) && userTasks.length > 0) {
      const firstTask = userTasks[0]
      navigate(`/pengisian-matriks-led/${firstTask.no}/${firstTask.sub}`, {
        replace: true, // Supaya tidak menambah entry baru di history browser
      })
    } else if (typeof setNumDesc === "function") {
      setNumDesc(false)
    }
  }
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUserProdi(userData.prodi)
      setSelectedProdi(userData.prodi)
  ...
  const handleInputChange = (index, type, field, value) => {
    ...
    setSelectedData((prev) => {
      ...
      const targetIndex = updatedDetails.findIndex(
        (detail) => detail.Type === type && detail.Seq === updatedIndex
      )
      ...
      if (targetIndex !== -1) {
        ...
        console.log(
          "Elemen tidak ditemukan dengan Type:",
          type,
          "dan Seq:",
          updatedIndex
        )
  ...
  const handleClickButton = async (index) => {
    ...
    const findDetailIndex = (type, seq = null) =>
      updatedDetails.findIndex(
        (detail) => detail.Type === type && (seq ? detail.Seq === seq : true)
      )
    ...
    if (targetIndexes.isianAsesiAndReferensiKriteria !== -1) {
      const isianAsesi =
        updatedDetails[targetIndexes.isianAsesiAndReferensiKriteria]?.[
          "Isian Asesi"
        ]
      ...
      const analysisPrompt = `
                ...
                Pertanyaan:
                Apakah isiian di atas memenuhi poin penilaian kriteria indikator "${
                  updatedDetails[targetIndexes.isianAsesiAndReferensiKriteria][
                    "Reference"
                  ]
                }"? Jika isian tidak relevan atau tidak memberikan informasi yang sesuai dengan kriteria penilaian, langsung berikan nilai 0 dan beri masukan 'Informasi tidak mencukupi untuk evaluasi.'
  ...
  const saveToFile = async (commit) => {
    ...
    try {
      const response = await axiosInstance.post("/save-json", {
        data: flattenData,
      })
  ...
  const countScore = (selectedDataReference) => {
    ...
    const pembagi = selectedDataReference.Details.filter(
      (detail) => detail.Type === "K"
    ).length
  ...
  return (
    <div className="h-[80vh] w-full pb-4 overflow-y-auto bg-white shadow-lg radius rounded-lg">
      ...
      <h2 className="text-3xl font-semibold mt-5 ml-2 text-center">
        {userProdi.toUpperCase()}
      </h2>
      <div className="flex justify-center space-x-4 mt-4">
        <DropdownWithSearch
          ...
          onChange={(newValue) => {
            setSelectedProdi({ name: newValue?.value || "" })
          }}
          disabled={isLoading}
                ...
                <table className="text-center w-full text-sm">
                  <tbody>
                    <tr className="border border-black black-600 text-center w-full">
                      <td
                        className="align-middle text-center w-full"
                        colSpan="3"
                      >
                        <h4>Guidance</h4>
                        <div>
                          {selectedData?.Details?.filter(
                            (detail) => detail.Type === "G"
                          ).map((detail, index) => (
                            <div key={index}>
                    ...
                    <tr>
                      ...
                      <td className="border border-black black-600 w-[33%]">
                        <h4>Deskripsi</h4>
                        <div>
                          {selectedData?.Details?.some(
                            (detail) => detail.Type === "D"
                          ) ? (
                            selectedData.Details.filter(
                              (detail) => detail.Type === "D"
                            ).map((detail, index) => (
                        ...
                        <div>
                          {viewAllVersion ? (
                            ...
                            <>
                              <h1 className="text-xl font-bold">
                                Commit :{" "}
                                {version[parseInt(versionSelected)].commit}
                              </h1>
                              <h3>
                                Diperbarui pada{" "}
                                {new Intl.DateTimeFormat("en-GB", {
                                  ...
                                  new Date(
                                    version[
                                      parseInt(versionSelected)
                                    ].created_at
                                  )
                                )}
                                , oleh{" "}
                                {version[parseInt(versionSelected)].user_name}
                              </h3>
                            </>
                          )}
                        </div>
